<p align="center">
  <img src="https://github.com/vishal316156/vishal316156/blob/main/assets/banner.svg" width="100%" alt="Banner">
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=vishal316156&label=Profile%20Views&color=6f42c1&style=for-the-badge" alt="Profile Views"/>
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/vishalkumarnitjsr/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>

  <a href="https://portfolio-vishal316156.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>

  <a href="mailto:vishalnitjsr2605ee@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

## 👨‍💻 About Me

Over time, solving problems naturally led to building projects, and every project pushed me to learn something new. What started with programming gradually grew into building full stack applications and exploring AI, where every challenge became an opportunity to learn, improve, and build something better.

Every project has taught me something different. Sometimes it's designing a better user experience, sometimes it's debugging an API that refuses to cooperate, and sometimes it's realizing there's a better way to build something than my first approach. Every challenge leaves me with something valuable to carry into the next project, and that's what keeps me coming back.

---

# 🚀 Things I've Built

### 🤖 AI ChatBot

> What began as an experiment with AI gradually evolved into a complete full stack application where users can chat, generate images, save conversations, and continue where they left off.

<p>
<a href="https://ai-chatbot-lilac-seven.vercel.app/">
<img src="https://img.shields.io/badge/🌐 Live Demo-000000?style=for-the-badge"/>
</a>

<a href="https://github.com/vishal316156/AiChatbot">
<img src="https://img.shields.io/badge/📂 Repository-24292F?style=for-the-badge"/>
</a>
</p>

<details>
<summary><b>More about this project</b></summary>

### Why I built it

I wanted to build something that felt like a real product instead of another tutorial project. The goal wasn't just to integrate AI, but to understand everything that goes into building a complete application.

### Highlights

- Secure authentication using JWT
- Persistent chat history
- AI image generation
- Responsive UI
- Clean and intuitive user experience

### What I learned

This project strengthened my understanding of backend architecture, API design, authentication, database design, media handling, and building applications that feel complete instead of just functional.

### Next Improvements

- Streaming AI responses
- Better backend architecture
- Automated testing
- Performance optimization

</details>

---

### 🌐 Portfolio Website

A personal portfolio built to showcase my projects, learning journey, and growth as a developer while focusing on clean UI and user experience.

<p>
<a href="https://portfolio-vishal316156.vercel.app/">
<img src="https://img.shields.io/badge/🌐 Live-000000?style=for-the-badge"/>
</a>

<a href="https://github.com/vishal316156/portfolio-Vishal316156">
<img src="https://img.shields.io/badge/📂 Repository-24292F?style=for-the-badge"/>
</a>
</p>

---

### 📝 Notes App

A simple notes application built with React and Redux Toolkit that focuses on state management, searching, and local persistence.

<p>
<a href="YOUR_NOTES_DEMO">
<img src="https://img.shields.io/badge/🌐 Live-000000?style=for-the-badge"/>
</a>

<a href="YOUR_NOTES_REPOSITORY">
<img src="https://img.shields.io/badge/📂 Repository-24292F?style=for-the-badge"/>
</a>
</p>

---

# 💻 Technologies I Work With

<p align="center">

<img src="https://skillicons.dev/icons?i=c,cpp,js,html,css,react,redux,tailwind,nodejs,express,mongodb,git,github,vscode,postman&perline=5" />

</p>

<p align="center">
<b>Also familiar with:</b> SQL • REST APIs • JWT Authentication • Gemini API
</p>

---

# 🧠 Problem Solving

Building projects teaches me how software comes together, while DSA teaches me how to think through problems. Both have shaped the way I approach software development.

<p align="center">

<a href="https://leetcode.com/u/vishal_kumar_ee_nitjsr/">
<img src="https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
</a>

<a href="https://codeforces.com/profile/Vishal_MarcoX_83">
<img src="https://img.shields.io/badge/Codeforces-1F8ACB?style=for-the-badge&logo=codeforces&logoColor=white"/>
</a>

<a href="https://www.geeksforgeeks.org/profile/vishalnitj54i6">
<img src="https://img.shields.io/badge/GeeksforGeeks-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>
</a>

</p>

---

# 🎯 Currently Working On

- [ ] Building stronger backend applications
- [ ] Learning cloud deployment
- [ ] Improving DSA and competitive programming
- [ ] Building more polished full stack applications

---
---

# 📊 GitHub Activity

<div align="center">

  <img height="170" src="https://github-readme-stats.vercel.app/api?username=vishal316156&show_icons=true&theme=github_dark&hide_border=true&rank_icon=github" />
  <img height="170" src="https://streak-stats.demolab.com?user=vishal316156&theme=github-dark&hide_border=true" />
</div>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=vishal316156&theme=github-dark&hide_border=true" />
</p>

---

# 🤝 Let's Connect

Thanks for stopping by! If you'd like to connect, collaborate, or simply talk about software, I'd be happy to hear from you.

<p align="center">

<a href="https://www.linkedin.com/in/vishalkumarnitjsr/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://portfolio-vishal316156.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<a href="mailto:vishalnitjsr2605ee@gmail.com">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</p>

---

<p align="center">
Thanks for visiting. I'm always learning, building, and looking for the next challenge.
</p>

